...
#### روش 1: استفاده از Username کانال
1. ربات را به کانال اضافه کنید (به عنوان Admin)
2. از username کانال استفاده کنید (مثلاً: `@my_trading_channel`)
3. یا از Chat ID عددی استفاده کنید (معمولاً منفی است، مثلاً: `-1001234567890`)

#### روش 2: دریافت Chat ID کانال
1. ربات را به کانال اضافه کنید (Admin)
2. یک پیام در کانال ارسال کنید
3. به این آدرس بروید:
   ```
   https://api.telegram.org/bot<YOUR_BOT_TOKEN>/getUpdates
   ```
4. در پاسخ JSON، `chat.id` کانال را پیدا کنید (معمولاً منفی است)
5. یا از username کانال استفاده کنید: `@channel_username`

**نکته:** برای کانال، Chat ID معمولاً به صورت `-1001234567890` است (عدد منفی)
