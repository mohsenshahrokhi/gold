...
# ============================================================================
# OPTIMIZED TRADING BOT - ربات معاملاتی بهینه‌شده
# ============================================================================
...
class ScalpingNDSTradingBot(OptimizedNDSTradingBot):
    """ربات اسکلپینگ حرفه‌ای"""
    ...
    def __init__(self, symbol: str = "BTCUSD", max_lots: float = None, config: Any = None):
        ...
        self.max_scalp_trades_per_hour = 10
        self.scalp_trade_count = 0
        self.last_scalp_reset = datetime.now()
        
        # تنظیم Risk Manager برای اسکلپ
        self.risk.max_risk_percent = 0.3  # ریسک کمتر برای اسکلپ
        self.risk.max_daily_loss = 0.01   # حداکثر 1% ضرر روزانه
        
        # حجم ثابت برای اسکلپ
        self.scalp_fixed_volume = 0.1  # حجم ثابت 0.1 لات
        
        logger.info("⚡ SCALPING NDS Trading Bot Initialized")
        logger.info("   Timeframes: M1/M2 (Ultra Fast)")
        logger.info("   Target: 10 pips, Max Risk: 5 pips")
        logger.info("   Max Trades/Hour: 10")
    
    def start(self):
        """شروع ربات اسکلپینگ"""
        logger.info("🚀 Starting SCALPING NDS Trading Bot...")
        logger.info("⚡ ULTRA FAST SCALPING MODE ACTIVATED")
        ...
        latency = (end_time - start_time) * 1000  # میلی‌ثانیه
        logger.info(f"   Connection Latency: {latency:.1f}ms")
        
        if latency > 100:
            logger.warning("⚠️ High latency detected! Scalping may be difficult")
        
        self.running = True
        logger.info("✅ Scalping Bot started successfully")
        
        try:
            self._scalp_main_loop()
        except KeyboardInterrupt:
            logger.info("Scalping Bot stopped by user")
        finally:
            self.stop()
    
    def _scalp_main_loop(self):
        """حلقه اصلی اسکلپینگ"""
        ...
        while self.running:
            try:
                ...
                if positions:
                    # مدیریت سریع پوزیشن‌های اسکلپ
                    self._manage_scalp_position(positions[0])
                else:
                    # ریست شمارنده هر ساعت
                    if (current_time - self.last_scalp_reset).seconds >= 3600:
                        self.scalp_trade_count = 0
                        self.last_scalp_reset = current_time
                    
                    # بررسی محدودیت معاملات
                    if self.scalp_trade_count >= self.max_scalp_trades_per_hour:
                        logger.info(f"⏸️ Hourly trade limit reached ({self.scalp_trade_count}/{self.max_scalp_trades_per_hour})")
                        time.sleep(10)
                        continue
                    
                    # تحلیل هر 10 ثانیه
                    if last_analysis_time is None or \
                       (current_time - last_analysis_time).seconds >= 10:
                        
                        signal = self._scalp_analysis_and_trade()
                        last_analysis_time = current_time
                
                # گزارش وضعیت هر 30 ثانیه
                if not hasattr(self, '_last_report') or \
                   (current_time - self._last_report).seconds >= 30:
                    self._scalp_status_report()
                    self._last_report = current_time
                
                time.sleep(0.5)  # خواب کوتاه برای اسکلپ
                
            except Exception as e:
                logger.error(f"Error in scalp main loop: {e}")
                time.sleep(2)
    
    def _scalp_analysis_and_trade(self) -> Optional[NDSSignal]:
        """تحلیل و معامله اسکلپ"""
        try:
            # بررسی امکان معامله
            can_trade, reason = self.risk.can_trade()
            if not can_trade:
            ...
            # تحلیل اسکلپ
            signal = self.nds.scalp_analyze() if hasattr(self.nds, 'scalp_analyze') else self.nds.optimized_analyze()
            ...
            if signal.risk_reward < 1.8:
                logger.info(f"R/R ({signal.risk_reward:.2f}) too low for scalp")
                return None
            
            if signal.confidence < 0.7:
                logger.info(f"Confidence ({signal.confidence:.2%}) too low for scalp")
                return None
            
            # تنظیم حجم ثابت برای اسکلپ
            volume = self.scalp_fixed_volume
            ...
            request = {
                ...
                "sl": signal.stop_loss,
                "tp": signal.take_profit,
                "deviation": 50,  # افزایش deviation برای اسکلپ
                "magic": 987654,  # مخصوص اسکلپ
                "comment": f"SCALP_{'BUY' if signal.direction == TrendDirection.BULLISH else 'SELL'}",
                "type_time": mt5.ORDER_TIME_GTC,
            }
            
            logger.info(f"📤 Sending SCALP order: {volume} lots @ {price:.2f}")
            result = mt5.order_send(request)
            ...
            if result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                logger.info(f"🎯 SCALP trade opened! Ticket: #{result.order}")
                logger.info(f"   Trades this hour: {self.scalp_trade_count}/{self.max_scalp_trades_per_hour}")
                return signal
            else:
                logger.error(f"Scalp trade failed: {result.retcode}")
                return None
                
        except Exception as e:
            logger.error(f"Error in scalp analysis and trade: {e}")
            return None
    
    def _manage_scalp_position(self, trade: TradeInfo):
        """مدیریت پوزیشن اسکلپ"""
        try:
            bid, ask = self.mt5.get_current_price()
            current_price = bid if trade.order_type == mt5.ORDER_TYPE_BUY else ask
            ...
            # بررسی سود/ضرر سریع
            profit_pips = abs(current_price - trade.open_price) / self.mt5.get_point()
            
            # قوانین بستن سریع اسکلپ
            current_profit = trade.profit
            
            # 1. سود سریع 5 پیپ
            if profit_pips >= 5 and current_profit > 0:
                self.trade.close_trade(trade, "Quick scalp profit (5 pips)")
                logger.info(f"💰 Quick scalp profit taken: {profit_pips:.1f} pips")
                return
            
            # 2. ضرر سریع 3 پیپ
            if profit_pips >= 3 and current_profit < 0:
                self.trade.close_trade(trade, "Scalp stop loss (3 pips)")
                logger.info(f"🛑 Scalp stop loss: {profit_pips:.1f} pips")
                return
            
            # 3. تارگت اصلی 10 پیپ
            if profit_pips >= 10:
                self.trade.close_trade(trade, "Scalp target reached (10 pips)")
                logger.info(f"🎯 Scalp target reached: {profit_pips:.1f} pips")
                return
            
        except Exception as e:
            logger.error(f"Error managing scalp position: {e}")
    
    def _scalp_status_report(self):
        """گزارش وضعیت اسکلپ"""
        self.mt5.refresh_account()
        positions = self.mt5.get_active_positions()
        bid, ask = self.mt5.get_current_price()
        ...
        logger.info("=" * 50)
        logger.info(f"⚡ SCALP STATUS - {datetime.now().strftime('%H:%M:%S')}")
        logger.info(f"   Balance: ${self.mt5.account_info.balance:,.2f}")
        logger.info(f"   Equity: ${self.mt5.account_info.equity:,.2f}")
        logger.info(f"   {self.symbol}: Bid={bid:.2f} | Ask={ask:.2f}")
        logger.info(f"   Trades this hour: {self.scalp_trade_count}/{self.max_scalp_trades_per_hour}")
        ...
        if positions:
            pos = positions[0]
            profit_pips = abs(pos.current_price - pos.open_price) / self.mt5.get_point()
            logger.info(f"   Active SCALP: {'BUY' if pos.order_type == mt5.ORDER_TYPE_BUY else 'SELL'}")
            logger.info(f"   P/L: ${pos.profit:.2f} ({profit_pips:.1f} pips)")
        else:
            logger.info("   Status: Waiting for scalp signal...")
        
        logger.info("=" * 50)


# ============================================================================
# PROFESSIONAL SCALPING BOT - ربات اسکلپینگ حرفه‌ای
# ============================================================================
# ============================================================================
# PROFESSIONAL SCALPING BOT - ربات اسکلپینگ حرفه‌ای (نسخه کاملاً اصلاح شده)
# ============================================================================

# ============================================================================
# PROFESSIONAL SCALPING BOT - ربات اسکلپینگ حرفه‌ای (نسخه پایدار و کامل)
# ============================================================================
class ProfessionalScalpingBot(OptimizedNDSTradingBot):
    """ربات اسکلپینگ حرفه‌ای - نسخه پایدار"""
    def _print_status_report(self):
        """چاپ گزارش وضعیت - نسخه اصلاح شده"""
        self.mt5.refresh_account()
        positions = self.mt5.get_active_positions()
...
# ============================================================================
def run_professional_scalping_bot():
...
def main():
    ...
    print("=" * 60)
    print("1. 🎯 PROFESSIONAL SCALPING BOT (RECOMMENDED)")
    print("   - Ultra fast scalping with advanced risk management")
    print("   - Best for: Active trading, quick profits")
    print()
    print("2. ⚡ Ultra Fast Scalping Bot")
    print("   - Fast scalping strategy")
    print("   - Best for: High frequency trading")
    print()
    print("3. 🚀 Optimized NDS Bot (Phase 2 - Performance)")
    print("   - Optimized with caching and performance improvements")
    print("   - Best for: Balanced performance and accuracy")
    print()
    print("4. 📈 Enhanced NDS Bot (Phase 1 - Complete)")
    print("   - Complete NDS implementation with all features")
    print("   - Best for: Full feature set")
    print()
    print("5. 🔧 Original NDS Bot (Basic)")
    print("   - Basic NDS implementation")
    print("   - Best for: Simple trading")
    print()
    print("6. 🔌 Test MT5 Connection Only")
    print("=" * 60)
    
    while True:
        try:
            choice = input("\nEnter strategy number (1-6): ").strip()
    ...
    try:
        if choice == "1":
            ...
            max_lots = config.scalping.fixed_volume if config else 0.5
            bot = ProfessionalScalpingBot(symbol=selected_symbol, max_lots=max_lots, config=config)
            bot.start()
        
        elif choice == "2":
            # ✅ Ultra Fast Scalping
            logger.info("⚡ Starting Ultra Fast Scalping Bot...")
            logger.info(f"   Symbol: {selected_symbol} ({selected_display})")
            max_lots = config.scalping.fixed_volume if config else 0.5
            bot = ScalpingNDSTradingBot(symbol=selected_symbol, max_lots=max_lots, config=config)
            bot.start()
        
        elif choice == "3":
            # ✅ Optimized Bot
            logger.info("🚀 Starting Optimized NDS Bot...")
            logger.info(f"   Symbol: {selected_symbol} ({selected_display})")
            max_lots = config.max_lots if config else 0.3
            bot = OptimizedNDSTradingBot(symbol=selected_symbol, max_lots=max_lots, config=config)
            bot.start()
        
        elif choice == "4":
            # ✅ Enhanced Bot
            logger.info("📈 Starting Enhanced NDS Bot...")
            logger.info(f"   Symbol: {selected_symbol} ({selected_display})")
            max_lots = config.max_lots if config else 0.3
            bot = EnhancedNDSTradingBot(symbol=selected_symbol, max_lots=max_lots, config=config)
            bot.start()
        
        elif choice == "5":
            # ✅ Original Bot
            logger.info("🔧 Starting Original NDS Bot...")
            logger.info(f"   Symbol: {selected_symbol} ({selected_display})")
            max_lots = config.max_lots if config else 0.3
            bot = NDSTradingBot(symbol=selected_symbol, max_lots=max_lots, config=config)
            bot.start()
