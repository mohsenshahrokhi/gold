...
class TradeManager:
    ...
    def _add_sltp_later(self, ticket: int, signal: NDSSignal):
        ...
        try:
            ...
            # تنظیم SL/TP با استفاده از مقادیر سیگنال
            point = self.mt5.get_point()
            
            if signal.direction == TrendDirection.BULLISH:
                # BUY: SL زیر قیمت باز شدن، TP بالای قیمت باز شدن
                sl_price = signal.stop_loss
                tp_price = signal.take_profit
                
                # اعتبارسنجی: اگر SL بالاتر از قیمت باز شدن باشد، تنظیم کن
                ...
                # اعتبارسنجی: اگر TP پایین‌تر از قیمت باز شدن باشد، تنظیم کن
                ...
                # SELL: SL بالای قیمت باز شدن، TP زیر قیمت باز شدن
                sl_price = signal.stop_loss
                tp_price = signal.take_profit
                
                # اعتبارسنجی: اگر SL پایین‌تر از قیمت باز شدن باشد، تنظیم کن
                if sl_price <= position.price_open:
                    logger.warning(f"⚠️ Signal SL ({sl_price:.2f}) <= Open Price ({position.price_open:.2f}), adjusting...")
                    sl_price = position.price_open + (10 * point)  # 10 پیپ بالاتر
                
                # اعتبارسنجی: اگر TP بالاتر از قیمت باز شدن باشد، تنظیم کن
                if tp_price >= position.price_open:
                    logger.warning(f"⚠️ Signal TP ({tp_price:.2f}) >= Open Price ({position.price_open:.2f}), adjusting...")
    ...
    def _add_sltp_comprehensive(self, ticket: int, signal: NDSSignal):
        ...
        try:
            ...
            if position.type == mt5.ORDER_TYPE_BUY:
                # برای BUY: استفاده از مقادیر سیگنال
                sl_price = signal.stop_loss
                tp_price = signal.take_profit
                ...
                # برای SELL: استفاده از مقادیر سیگنال
                sl_price = signal.stop_loss
                tp_price = signal.take_profit
...
class OptimizedNDSAnalyzer(EnhancedNDSAnalyzer):
    ...
    def _enhance_signal_with_article_models(self, signal: NDSSignal, 
        ...
        try:
            ...
            # 2. استفاده از GARCH برای تنظیم SL (فقط اگر R/R را بهبود دهد)
            if 'garch_volatility' in article_signals:
                ...
                # محاسبه حداقل فاصله SL (بر اساس فاصله اصلی)
                original_sl_distance = abs(entry - original_sl)
                min_sl_distance = original_sl_distance * 0.5  # حداقل 50% فاصله اصلی
                max_sl_distance = original_sl_distance * 1.5  # حداکثر 150% فاصله اصلی
                
                # تنظیم SL بر اساس نوسان GARCH (با محدودیت)
                k = 1.5  # Multiplier
                if signal.direction == TrendDirection.BULLISH:
                    # محاسبه SL جدید
                    ...
                    # فقط اگر SL جدید نزدیک‌تر به entry باشد (بهتر) و R/R را حفظ کند
                    if new_sl > original_sl:  # برای BUY، SL بهتر = بالاتر (نزدیک‌تر)
                        # بررسی R/R جدید
                        new_rr = abs(signal.take_profit - entry) / abs(entry - new_sl)
                        if new_rr >= 1.5 and new_rr >= original_rr * 0.9:  # حداقل R/R 1.5 و حداقل 90% R/R اصلی
                    ...
                    # برای SELL، SL بهتر = پایین‌تر (نزدیک‌تر)
                ...
                # اگر R/R کاهش یافت، تغییر را برگردان
                if signal.risk_reward < 1.5:
...
class UnifiedTradingBot(OptimizedNDSTradingBot):
    ...
    def __init__(self, symbol: str = "BTCUSD", max_lots: float = None, 
        ...
        # ⭐ Override متد محاسبه سطوح برای استفاده از استراتژی
        # ذخیره متد اصلی
        self._original_calculate_levels = self.nds._calculate_levels_simple
        # جایگزینی با متد جدید - استفاده از lambda برای capture کردن UnifiedTradingBot instance
        bot_instance = self  # Capture the UnifiedTradingBot instance
        self.nds._calculate_levels_simple = lambda direction, df: bot_instance._calculate_levels_strategy_based(direction, df)
        
        # ⭐ Initialize exit signal tracking
        ...
        # ⭐ بررسی و تنظیم SL/TP برای معاملات باز موجود
        self._check_and_set_sltp_for_open_positions()
    
    def _check_and_set_sltp_for_open_positions(self):
        """
        بررسی معاملات باز در زمان راه‌اندازی و تنظیم SL/TP برای آن‌هایی که SL/TP ندارند
        """
        try:
            logger.info("🔍 Checking for open positions without SL/TP...")
            
            # دریافت معاملات باز برای نماد فعلی
            positions = mt5.positions_get(symbol=self.symbol)
            ...
            # دریافت داده‌های قیمت برای محاسبه SL/TP
            ...
            for position in positions:
                # بررسی اینکه آیا SL یا TP ست نشده است
                if position.sl == 0.0 or position.tp == 0.0:
                    ...
                    # تعیین جهت بر اساس نوع معامله
                    direction = TrendDirection.BULLISH if position.type == mt5.ORDER_TYPE_BUY else TrendDirection.BEARISH
                    
                    # استفاده از قیمت باز شدن معامله به عنوان entry
                    entry_price = position.price_open
                    
                    # محاسبه SL/TP بر اساس استراتژی (با استفاده از قیمت فعلی برای محاسبه ATR)
                    entry, sl, tp = self._calculate_levels_strategy_based(direction, df)
                    
                    if entry > 0 and sl > 0 and tp > 0:
                        # محاسبه فاصله SL/TP از entry (که در _calculate_levels_strategy_based محاسبه شده)
                        current_price = df['close'].values[-1]
                        
                        # تنظیم SL/TP بر اساس قیمت باز شدن معامله
                        if position.type == mt5.ORDER_TYPE_BUY:
                            # برای BUY: SL باید زیر entry باشد، TP بالای entry
                            # محاسبه فاصله از entry در محاسبه اصلی
                            sl_distance = entry - sl  # فاصله SL از entry در محاسبه اصلی
                            tp_distance = tp - entry  # فاصله TP از entry در محاسبه اصلی
                            ...
                            # برای SELL: SL باید بالای entry باشد، TP زیر entry
                            sl_distance = sl - entry  # فاصله SL از entry در محاسبه اصلی
                            tp_distance = entry - tp  # فاصله TP از entry در محاسبه اصلی
    ...
    def _calculate_levels_strategy_based(self, direction: TrendDirection, df: pd.DataFrame) -> Tuple[float, float, float]:
        """
        محاسبه SL/TP بر اساس استراتژی انتخاب شده
        برای سوپر اسکلپ: SL/TP کوچکتر
        برای Day Trading: SL/TP بزرگتر
        """
        try:
            current_price = df['close'].values[-1]
            ...
            # تعیین ضرایب بر اساس استراتژی
            if self.strategy_name == "super_scalping":
                # سوپر اسکلپ: SL/TP کوچک و سریع
                sl_multiplier = 0.5   # 0.5x ATR برای SL
                tp_multiplier = 1.0   # 1.0x ATR برای TP
                min_rr = 1.5          # حداقل R/R
            elif self.strategy_name == "scalping":
                # اسکلپینگ: SL/TP متوسط
                sl_multiplier = 1.0   # 1.0x ATR برای SL
                tp_multiplier = 2.0   # 2.0x ATR برای TP
                min_rr = 1.5
            else:  # day_trading
                # Day Trading: SL/TP بزرگتر
                sl_multiplier = 1.5   # 1.5x ATR برای SL
                tp_multiplier = 3.0   # 3.0x ATR برای TP
                min_rr = 1.5
            ...
            if direction == TrendDirection.BULLISH:
                ...
                if rr < min_rr:
                    # اگر R/R کم است، TP را افزایش بده
    ...
    def _check_exit_signal(self) -> bool:
        ...
        try:
            ...
            positions = self.mt5.get_active_positions()
            if not positions:
                # Reset exit signal flag if no positions
            ...
            for pos in positions:
                ...
                if trend_signal != trade_direction and trend_signal != TrendDirection.NEUTRAL:
                    if trend_confirm == trend_signal:  # تایید در confirm
                        # جلوگیری از تکرار پیام - فقط یک بار چاپ کن
                        if not hasattr(self, '_exit_signal_logged'):
                            self._exit_signal_logged = {}
                        
                        # بررسی اینکه آیا این یک سیگنال جدید است یا قبلاً ثبت شده
                        if pos.ticket not in self._exit_signal_logged:
                            # سیگنال جدید - چاپ پیام و ثبت
                            ...
                            return True  # فقط برای اولین بار True برمی‌گرداند
                        else:
                            # سیگنال قبلاً ثبت شده - فقط هر 30 ثانیه یک یادآوری
                            ...
                            if time_diff >= 30:
                                ...
                                self._exit_signal_logged[pos.ticket] = datetime.now()
                            # ⭐ مهم: False برمی‌گرداند چون سیگنال جدید نیست
                            return False
                else:
                    # اگر روند برگشت به حالت اولیه، flag را پاک کن
        ...
        while self.running:
            try:
                ...
                if positions:
                    ...
                    # فقط اگر سیگنال جدید باشد، اقدام به بستن معامله کن
                    if exit_signal_detected:
                        for pos in positions:
                            # بررسی اینکه آیا این معامله برای خروج علامت‌گذاری شده (سیگنال جدید)
                            if hasattr(self, '_exit_signal_logged') and pos.ticket in self._exit_signal_logged:
                                # بستن معامله در هر حالتی که روند تغییر کرده (نه فقط در سود)
                                # اما اگر در ضرر شدید باشد، صبر می‌کنیم تا SL فعال شود
                                if pos.profit > 0:
                                    ...
                                    # حذف از لیست exit signals بعد از بستن
                                    if pos.ticket in self._exit_signal_logged:
                                        del self._exit_signal_logged[pos.ticket]
                                elif pos.profit < -50:  # اگر ضرر بیشتر از 50 دلار باشد
...
def main():
    ...
    def get_input_with_timeout(prompt: str, timeout: int = 10, default_value: str = None) -> str:
        ...
        if input_thread.is_alive():
            # Timeout - استفاده از مقدار پیش‌فرض
            if default_value:
    ...
    # تعیین نماد پیش‌فرض: اول YM (اگر فعال باشد)، بعد BTC
