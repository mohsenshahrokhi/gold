...
class OptimizedGoldenmanBot(EnhancedGoldenmanBot):
    ...
    def __init__(self, symbol: str = "BTCUSD", max_lots: float = None, config: Any = None):
        ...
        if not telegram_token and config and hasattr(config, 'telegram'):
            telegram_token = config.telegram.get('bot_token') if isinstance(config.telegram, dict) else None
        if not telegram_chat_id and config and hasattr(config, 'telegram'):
            telegram_chat_id = config.telegram.get('chat_id') if isinstance(config.telegram, dict) else None
